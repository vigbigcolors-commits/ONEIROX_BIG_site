...
function cta(page) {
  const renderedLinks = page.cta.links
    .map((link) => {
...
function assertRendered(page, html) {
  ...
  if ((html.match(/<h1\b/g) || []).length !== 1) {
    throw new Error(`Expected one H1 for ${page.slug}`);
  }
...
function assertHub(html) {
  ...
  for (const token of required) {
    if (!html.includes(token)) throw new Error(`REM hub missing: ${token}`);
  }

  if ((html.match(/<h1\b/g) || []).length !== 1) {
    throw new Error("REM hub must contain exactly one H1");
  }

  if (/{{[^}]+}}/.test(html)) {
    throw new Error("REM hub contains unresolved token");
  }
}

function main() {
  const source = JSON.parse(fs.readFileSync(SOURCE, "utf8"));
  const hubHtml = fs.readFileSync(HUB_SOURCE, "utf8");

  assertHub(hubHtml);

  const rendered = source.pages.map((page) => ({
    page,
    html: renderRemMechanicsPage(page),
  }));

  rendered.forEach(({ page, html }) => assertRendered(page, html));

  if (check) {
    console.log(`CHECK hub: ${hubHtml.length} bytes`);
  ...
  for (const name of ["rem-mechanics.css", "rem-mechanics.js"]) {
    fs.copyFileSync(
      path.join(ASSET_SOURCE, name),
      path.join(OUT, "assets", name)
    );
  }

  const hubTarget = path.join(OUT, "rem", "index.html");
  fs.mkdirSync(path.dirname(hubTarget), { recursive: true });
  fs.writeFileSync(hubTarget, hubHtml);
  console.log(`Wrote ${path.relative(ROOT, hubTarget)}`);
